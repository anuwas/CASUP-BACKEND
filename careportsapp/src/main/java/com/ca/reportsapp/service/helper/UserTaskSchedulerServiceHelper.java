...
public class UserTaskSchedulerServiceHelper {
	...
	public Page<UserTaskScheduler> retrieveReleaseItemByDTORequest(UserTaskSchedulerRequest filter, Pageable pageable) {
        ...
        if(filter.getTaskScheduledStartDate()!=null && filter.getTaskScheduledEndDate()==null) {		
        	predicates.add(builder.greaterThanOrEqualTo(supportItemRoot.get("taskScheduledDate"), filter.getTaskScheduledStartDate()));
        }
        
        if(filter.getTaskScheduledStartDate()==null && filter.getTaskScheduledEndDate()!=null) {		
        	predicates.add(builder.lessThanOrEqualTo(supportItemRoot.get("taskScheduledDate"), filter.getTaskScheduledEndDate()));
        }
