...
public class UserTaskController {
	...
	@PostMapping("/save-scheduled-task-activity")
	public ResponseEntity<Object> createScheduledTaskActivity(@RequestBody UserTaskSchedulerActivity userTaskSchedulerActivity,UriComponentsBuilder builder) {
		UserTaskSchedulerActivity savedItem = userTaskSchedulerService.saveTaskSchedulerActivity(userTaskSchedulerActivity);

		HttpHeaders headers = new HttpHeaders();
        headers.setLocation(builder.path("/view-scheduled-task-activity/{taskSchedulerActivityId}").buildAndExpand(savedItem.getTaskSchedulerActivityId()).toUri());
        return new ResponseEntity<Object>(headers,HttpStatus.CREATED);
	}
	
	@GetMapping("/all-scheduled-task-activity-list/{taskId}")
	public List<UserTaskSchedulerActivity> retriveAllScheduledTaskActivity(@PathVariable long taskId) {
		 return userTaskSchedulerService.getAllUserTaskSchedulerActivity(taskId);
	}
	
	@GetMapping("/view-scheduled-task-activity/{taskSchedulerActivityId}")
	public UserTaskSchedulerActivity retrieveScheduledTaskActivity(@PathVariable long taskSchedulerActivityId) {
		return userTaskSchedulerService.getUserTaskSchedulerActivityById(taskSchedulerActivityId);
	}
