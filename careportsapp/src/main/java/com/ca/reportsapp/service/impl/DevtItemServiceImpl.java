...
public class DevtItemServiceImpl implements DevtItemService{
	...
	public DevItem saveItem(DevItem item) {
		if(item.getParentItem()!=null && !item.getParentItem().equals("")) {
			long parentItemId =  devItemDAO.findByItemNumber(item.getParentItem().trim()).get(0).getDevItemId();
			item.setItemParentId(parentItemId);
			item.setItemGroup(item.getParentItem());
		}else {
			item.setItemGroup(item.getItemNumber());
		}
	...
	@Override
	public Page<DevSprint> getDevSprintsByRequest(String srcDevSprint, int pageNumber) {
		DevSprintSrcRequest devSprintSrcRequest = gson.fromJson(srcDevSprint, DevSprintSrcRequest.class);
		Pageable pageable = PageRequest.of(pageNumber-1, 50);
		return devItemServiceHelper.retrieveDevSprintByDTORequest(devSprintSrcRequest, pageable);
	}

	@Override
	public DevSprint getDevSprintById(long devSprintId) {
		return devSprintRepository.findById(devSprintId).get();
	}

	@Override
	public DevSprint saveDevSprint(DevSprint devSpint) {
		return devSprintRepository.save(devSpint);
	}

	@Override
	public void deleteDevSprint(long devSprintId) {
		devSprintRepository.deleteById(devSprintId);
